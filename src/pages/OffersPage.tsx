  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-4xl mx-auto px-3 sm:px-8 text-center">
        ...
        >
          <Button size="lg" onClick={() => scrollTo('offers-sessions')}>
    ...
    <SectionWrapper id="offers-sessions">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 max-w-5xl mx-auto">
        {sessions.map((session, i) => {
          const cap = capacityStyles[session.capacity]
          return (
            <motion.div
              key={session.id}
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span
                  className={`shrink-0 inline-flex items-center px-2.5 py-1 rounded-full text-xs font-heading font-medium border ${cap.className}`}
                >
                  {cap.label}
                </span>
              </div>
              ...
              <Button
                variant={cap.available ? 'primary' : 'secondary'}
                className="w-full mt-auto"
                href={cap.available ? session.lineUrl : siteConfig.lineUrl}
