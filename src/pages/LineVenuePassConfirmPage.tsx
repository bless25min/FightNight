...
function getFriendRequestStorageKey(referenceId: string) {
  return `ufcgym:venue-pass-line-friend-request:${referenceId}`
}
...
export function LineVenuePassConfirmPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        track({
          event: 'venue_pass_line_confirm_start',
          params: {
            reference_id: referenceId,
          },
          lineEventName: 'VenuePassLineStart',
        })
        ...
        if (!liff.isLoggedIn()) {
          ...
          track({
            event: 'venue_pass_line_confirm_login_redirect',
            params: { reference_id: referenceId },
            lineEventName: 'VenuePassLineLogin',
          })
        ...
        if (!isFriend) {
          track({
            event: 'venue_pass_line_confirm_friend_required',
            params: { reference_id: referenceId },
            lineEventName: 'VenuePassAddFriend',
          })
          return
        }
        ...
        const response = await fetch('/api/venue-pass-lead/line-confirm', {
          method: 'POST',
          headers: {
        ...
        if (notifyDelivered) {
          track({
            event: 'venue_pass_line_confirm_success',
            params: {
              reference_id: referenceId,
              line_notify_status: notifyStatus,
            },
            metaStandardEvent: 'Lead',
            lineEventName: 'VenuePassConfirmed',
          })
          window.setTimeout(() => {
          ...
          track({
            event: 'venue_pass_line_confirm_pending',
            params: {
              reference_id: referenceId,
              line_notify_status: notifyStatus,
            },
            lineEventName: 'VenuePassConfirmPending',
          })
        } else {
          track({
            event: 'venue_pass_line_confirm_partial',
            params: {
              reference_id: referenceId,
              line_notify_status: notifyStatus,
            },
            lineEventName: 'VenuePassConfirmPartial',
          })
        }
      } catch (confirmError) {
        const message =
        ...
        track({
          event: 'venue_pass_line_confirm_error',
          params: {
            reference_id: referenceId,
            error_message: message,
          },
          lineEventName: 'VenuePassLineError',
        })
      }
    }
  ...
  const handleFriendRetry = async () => {
    ...
    track({
      event: 'venue_pass_line_confirm_friend_retry',
      params: { reference_id: referenceId },
      lineEventName: 'VenuePassAddFriendRetry',
    })
          ...
          <div className="mt-7 flex flex-col gap-3 sm:flex-row">
            {shouldShowLineReturnButton && (
              <Button
                ...
                size="lg"
                className="w-full"
                data-cta="venue-pass-line-confirm-close"
                onClick={() => {
                  track({
                    event: 'venue_pass_line_confirm_close',
                    params: {
                      reference_id: referenceId,
                      confirm_status: status,
                      line_notify_status: lineNotifyStatus,
                    },
                    lineEventName: 'VenuePassLineClose',
                  })
                  closeLiffWindow()
                }}
              >
            ...
            {shouldShowFriendRetryButton && (
              <Button
                ...
                size="lg"
                className="w-full"
                data-cta="venue-pass-line-confirm-friend-retry"
            ...
            {shouldShowSupportButton && (
              <Button
                ...
                size="lg"
                className="w-full"
                data-cta="venue-pass-line-confirm-open-chat"
                onClick={() =>
                  track({
                    event: 'venue_pass_line_confirm_open_chat',
                    params: {
                      reference_id: referenceId,
                      confirm_status: status,
                      line_notify_status: lineNotifyStatus,
                    ...
                    lineEventName: 'VenuePassLineChat',
                  })
                }
              >
                找專員協助確認
            ...
            <Button
              ...
              size="lg"
              className="w-full"
              data-cta="venue-pass-line-confirm-home"
            >
              回到活動頁
            </Button>
          </div>
