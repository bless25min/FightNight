const VENUE_PASS_STATUS = 'venue_pass_lead'
...
async function notifyLineVenuePassLead(env, referenceId, options) {
  const lineNotify = await import('../shopline/line-notify.js')
  return lineNotify.notifyLineVenuePassLead(env, referenceId, options)
}
...
function toStoredLineContext(lineContext) {
  return {
    ...
    source: 'venue_pass_line_confirm',
  }
}
...
export async function confirmVenuePassLineReservation({
  env,
  referenceId,
  lineContext,
  upsertLineCustomer = upsertLineCustomerFromLineConfirm,
  notifyLineVenuePassLead: notifyLine = notifyLineVenuePassLead,
}) {
  ...
  if (!order) {
    return { ok: false, status: 404, error: 'Venue pass lead not found' }
  }
  if (order.status !== VENUE_PASS_STATUS) {
    return {
      ok: false,
      status: 400,
      error: 'Only venue pass leads can be confirmed through this flow',
    }
  }
  ...
  if (existingLineUserId && existingLineUserId !== lineUserId) {
    return {
      ok: false,
      status: 409,
      error: 'Venue pass lead is already linked to another LINE user',
    }
  }
  ...
  if (!update.meta?.changes) {
    return {
      ok: false,
      status: 409,
      error: 'Venue pass lead could not be linked to this LINE user',
    }
  }
